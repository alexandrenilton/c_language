...
void insereApostaLista(stLista* ptLista, stAposta* ptAposta)
{
	...
	//continua caso 2, porem se o estado for igual e a cidade for menor
	if ( strcmp(ptAposta->strEstado, ptLista->ptInicio->strEstado) == 0 )
	{
	...
	//caso 3, elemento a ser inserido é maior
	...
	aux = ptLista->ptFim;
	//se estado é maior que estado do final, insere apos o atual final
	...
	//se estado é igual ao estado final, verifica se o nome da cidade eh maior para inserir apos o atual final.
	if ( strcmp(ptAposta->strEstado, aux->strEstado) == 0 )
	{
...
void geraRelatorio(stLista* ptLista, int* ptVetor)
{
	...
	//Crio um ponteiro para um arquivo texto, escrita. "wt" sobrescreve 
	//qualquer arquivo que esteja com mesmo nome e diretorio apresentando abaixo.
	arquivoSaida = fopen("D:\\relatorio_pre.txt", "wt");

	if (ptVetor == NULL ){
		fprintf(stderr, "===> ATENCAO: Ainda nao houve sorteio.\n");
		fprintf(stderr, "===> -> Favor realizar sorteio pela Opcao 2\n");
		return;
	}
	...
	for(iIndice=0; iIndice < ptLista->iTotal; iIndice++)
	{
		...
		{
			/*procura quantas vezes uma determinada cidade/estado 
			  aparece na lista e traz o total na struct. */
	...
	/*Escreve relatorio no arquivo em si, de acordo com arquivo de saida proposto*/
	if ( ptLista->ptInicio != NULL ) 
	{
		...
		//faz calculos para saber a divisão da premiacao.
		...
		if ( ganhadores != NULL )
		{
			...
			/*Se nenhum ganhador nao for encontrado, escreve a msg abaixo no relatório*/
			fprintf(arquivoSaida, "Nenhum ganhador na SENA para esse sorteio\n" );
		}
		aposta = ptLista->ptInicio;
	}else{
		printf("Lista vazia\n");
...
/*
 * Esta função retorna a quantidade de numeros acertados,
 * basicamente ela verifica quantos numeros jogados pertencem a numeros sortiados.
 *
 */
